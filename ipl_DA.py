...
data = pd.read_csv(r"C:/Users/Pavan Deep/Desktop/ipl_data/cleaned.csv")
df = pd.DataFrame(data, index = None)
dff = pd.DataFrame(columns = ['batsman','batsman_runs','Strike_rate','Average','value'],index=None)
after_15 = df.loc[df['over']>13]
for y in after_15['batsman'].unique():

    kohli = after_15.loc[after_15['batsman']==y]
    scores_list=[]
    outs =0
    for x in kohli['id'].unique():
        match = kohli.loc[kohli['id']==x]
        full_dhoni = match.loc[(match['batsman']==y)]
        count =0
        sum = 0
    ...
    listed = [y,total_scores,strike_rate,average,Value]
    dff.loc[len(dff)] = listed
    dff.index = [""] * len(dff)
print(dff.sort_values(by = ['batsman_runs','value'],ascending=False))
